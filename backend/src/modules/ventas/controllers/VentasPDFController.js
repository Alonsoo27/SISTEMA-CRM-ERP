...
class VentasPDFController {
    ...
    static async exportarVentaDetalle(req, res) {
        try {
            ...
            const pdfPath = await VentasPDFController.generarPDFVenta(ventaData);
...
fileStream.on('error', (error) => {
    console.error('❌ Error en fileStream:', error);
    if (!res.headersSent) {
        res.status(500).json({ success: false, error: 'Error enviando archivo' });
    }
});
